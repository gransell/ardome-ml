...
namespace olib
{
   namespace opencorelib
	...
	namespace xml {
		...
		CORE_API xerces_string from_string(const char *source, unsigned int length);

		/// Convert a std::string to xml string
		/** Uses str_util::to_wstring */
		CORE_API xerces_string from_string(const std::string &source);

		/// Convert a wchar_t array to xml string
		/** Mac and Linux implementation repacks from 32 bit to 16 bit */
		CORE_API xerces_string from_string(const wchar_t *source, unsigned int length);

		/// Convert a std::wstring to xml string
		/** Mac and Linux implementation repacks from 32 bit to 16 bit */
		CORE_API xerces_string from_string(const std::wstring &source);

		/// Convert a Xerces string to std::wstring
		CORE_API std::wstring to_wstring(const XMLCh *source);

		/// Convert a Xerces string to olib::t_string
		CORE_API t_string to_t_string(const XMLCh *source);

		/// Convert a Xerces string to olib::t_string
		CORE_API t_string to_t_string(const XMLCh *source, unsigned int length);

		/// Convert a Xerces string to std::string
		/** Uses str_util::to_string */ 
		CORE_API std::string to_string(const XMLCh *source);
