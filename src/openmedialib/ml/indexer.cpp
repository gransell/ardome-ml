...
static indexer_job_ptr indexer_job_factory( const pl::wstring &url, index_type::type file_type, boost::uint16_t v4_index_entry_type = 0 );
...
static indexer_job_ptr indexer_job_factory( const pl::wstring &url, index_type::type file_type, boost::uint16_t v4_index_entry_type )
{
	if ( url.find( L"index:" ) != 0 )
	{
		if( file_type == index_type::awi )
		{
	...
	{
		ARENFORCE_MSG( file_type == index_type::media, "Got index generation request for non-media file \"%1%\"" )( url );
...
class indexer
{
	public:
		...
		indexer_item_ptr request( const pl::wstring &url, index_type::type file_type, boost::uint16_t v4_index_entry_type = 0 )
		{
			boost::recursive_mutex::scoped_lock lock( mutex_ );
...
indexer_item_ptr ML_DECLSPEC indexer_request( const openpluginlib::wstring &url, index_type::type file_type, boost::uint16_t v4_index_entry_type )
{
