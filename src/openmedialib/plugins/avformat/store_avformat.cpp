...
class ML_PLUGIN_DECLSPEC avformat_store : public store_type
{
	...
	private:
		...
		AVCodec *obtain_video_codec( )
		{
			...
			{
				codec = avcodec_find_encoder_by_name( pl::to_string( prop_vcodec_.value< pl::wstring >( ) ).c_str( ) );
				ARENFORCE_MSG( codec, "Unable to find video codec for %s" )( prop_vcodec_.value< pl::wstring >( ) );
				codec_id = codec->id;
			}
			else
			{
		...
		AVCodec *obtain_audio_codec( )
		{
			...
			{
				codec = avcodec_find_encoder_by_name( pl::to_string( prop_acodec_.value< pl::wstring >( ) ).c_str( ) );
				ARENFORCE_MSG( codec, "Unable to find audio codec for %s" )( prop_acodec_.value< pl::wstring >( ) );
				codec_id = codec->id;
			}
			else
			{
