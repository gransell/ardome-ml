	...
	protected:
		...
		void do_fetch( frame_type_ptr &result )
		{
			...
			if ( get_position( ) != expected_ && is_seekable( ) )
				url_fseek( context_, offset_ + boost::int64_t( get_position( ) ) * ( bytes_ + extra_ ), SEEK_SET );
			else
				seek( expected_ );
...
class ML_PLUGIN_DECLSPEC input_aud : public input_type
{
	public:
		...
		// Audio/Visual
		virtual int get_frames( ) const { return frames_; }
		virtual bool is_seekable( ) const { return context_ ? !context_->is_streamed : !prop_stream_.value< int >( ); }
	...
	protected:
		...
		void do_fetch( frame_type_ptr &result )
		{
			...
			{
				error = get_buffer( context_, ( unsigned char * )( &samples ), sizeof( samples ) );
			}
		...
		bool push( frame_type_ptr frame )
		{
			...
			if ( audio != 0 )
			{
				if ( prop_stream_.value< int >( ) )
				{
					...
					success = url_write( context_, ( unsigned char * )( &samples ), sizeof( samples ) );
				}
				if ( success )
