...
namespace
{
	...
	void initilize_array(uint8_t *arr, int size)
	{
...
{	
	...
	initilize_array(test_src, sizeof( test_src ));
...
{	
	...
	uint8_t *test_buf = static_cast< uint8_t * >( olib::opencorelib::utilities::aligned_alloc( 16, 32 ) );
	initilize_array(test_buf, 32);
	uint8_t test_orig_values[32];
	memcpy(test_orig_values, test_buf, 32);

	au::byteswap16_inplace(test_buf, 32);
	...
	olib::opencorelib::utilities::aligned_free( test_buf );
}
